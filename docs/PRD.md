...
**实现状态：**已接入服务端 Pi Agent、图片与口播模型、异步任务、FFmpeg 切镜检测与导出、背景音乐混音、帐号密码登录、Resend 注册邮箱验证码、Logo 菜单及模型管理后台。会话、素材、任务、成片与偏好按帐号隔离；同一帐号登录其他设备可继续使用服务端数据。画面语义理解与自助找回密码尚未提供；一条复合指令目前只创建一个主任务，复杂请求宜分步发送。详细证据和差距见[体验验收记录](ACCEPTANCE.md)。
...
管理后台还提供「HTML 视频特效」分区。管理员可复制、新建、编辑、启停特效模板，修改 HTML、时间轴、默认文案和画幅；保存前可在沙盒中预览，保存后可渲染、查看历史任务并下载 MP4。普通用户通过对话调用已启用模板，生成的视频进入当前会话的成片与素材库，便于继续拼接。模板源码与渲染记录在服务端持久化。
...
8. 用户可在对话中说「生成阳光开场特效视频，标题『去看更大的世界』」，也可附加图片调用「照片推镜」。系统从已启用 HTML 模板填入文案与图片，渲染 24 FPS MP4；结果进入该会话的成片库和素材库，后续可作为剪辑素材继续使用。

## 4. 原型图与页面需求

### 4.0 Landing、登录与注册

未登录时先展示暖色、简洁的 Landing：一句话说明“对话剪辑”能力，用真实产品语境展示旅行视频、对话指令和片段轨道。主操作进入注册，已有帐号可登录。注册填写显示名称、邮箱、Resend 邮件中的 6 位验证码与两次密码；验证码 10 分钟有效，发送后 60 秒可重发。登录填写邮箱和密码。成功后进入当前帐号的对话工作区，刷新保持登录；退出后返回 Landing。密码和验证码仅以服务端加盐哈希保存，浏览器使用 HttpOnly、SameSite 登录 Cookie。已有会话、素材、任务和成片只对归属帐号可见，未知 ID 返回 404。用户中心可修改密码，修改后其他登录会话失效。

![Landing、登录与注册视觉稿](auth-concept.png)
...
理念是「Write HTML. Render video」：HTML 是可编辑画面，暂停的 GSAP 时间轴是可寻址动效，HyperFrames 负责逐帧渲染和 MP4 编码。轻剪提供自己的 `QJMotion` 组件：文字入场、卡片弹出、强调线绘制、照片推镜、遮罩揭示和快速收束。后台编辑模板、预览并下载；对话只注入安全编码的标题、副标题和图片 URL，不执行用户输入的代码。首批三个模板为「阳光开场」「照片推镜」「故事收束」。详见 [HTML 特效实现与使用](HTML_EFFECTS.md) 和仓库内 `skills/qingjian-html-video/SKILL.md`。
...
| HTML 特效 | 后台可增加、编辑、启停和预览模板，渲染历史可下载；对话可用模板生成 MP4 并加入素材库。输出画幅、时长、帧率与可读性经实际文件和代表帧检查。 |
| 帐号与隔离 | 可注册、登录、保持登录、修改密码与退出；未登录 API 返回 401；跨帐号会话、素材、任务、成片与下载均不可访问。 |
| 用户中心与设置 | 入口可访问；显示帐号名称与邮箱；默认对话模型可选且新会话使用所选模型；语言切换后界面文案更新且选择得以保留；用户协议和隐私政策可分别打开；对话背景图片可预览、更换和恢复默认，且不影响成片。 |
...
- 当前服务限制为一次最多上传 6 个文件、每个 300 MB；剪辑方案最多 8 个片段、总长 60 秒，支持 9:16、16:9 和 1:1。帐号使用服务端持久存储，新注册需邮箱验证；自助密码找回与存储配额仍需后续建设。
