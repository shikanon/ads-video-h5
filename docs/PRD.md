...
**实现状态：**已接入服务端 Pi Agent、图片与口播模型、异步任务、FFmpeg 切镜检测与导出、背景音乐混音、帐号密码登录、Logo 菜单及模型管理后台。会话、素材、任务、成片与偏好按帐号隔离；同一帐号登录其他设备可继续使用服务端数据。画面语义理解与邮箱验证/自助找回密码尚未提供；一条复合指令目前只创建一个主任务，复杂请求宜分步发送。详细证据和差距见[体验验收记录](ACCEPTANCE.md)。
...
未登录时先展示暖色、简洁的 Landing：一句话说明“对话剪辑”能力，用真实产品语境展示旅行视频、对话指令和片段轨道。主操作进入注册，已有帐号可登录。注册填写显示名称、邮箱与两次密码；登录填写邮箱和密码。成功后进入当前帐号的对话工作区，刷新保持登录；退出后返回 Landing。密码仅以服务端 scrypt 哈希保存，浏览器使用 HttpOnly、SameSite 登录 Cookie。已有会话、素材、任务和成片只对归属帐号可见，未知 ID 返回 404。用户中心可修改密码，修改后其他登录会话失效。
...
- 当前服务限制为一次最多上传 6 个文件、每个 300 MB；剪辑方案最多 8 个片段、总长 60 秒，支持 9:16、16:9 和 1:1。帐号使用服务端持久存储，邮箱验证、自助密码找回与存储配额仍需后续建设。
