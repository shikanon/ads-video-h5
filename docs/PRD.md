...
**实现状态：**本文描述四张原型图对应的目标体验。当前仓库已实现视频上传、文字对话生成片段方案、FFmpeg 合成 MP4、预览和本地下载，但当前页面仍保留独立的「生成成片」按钮；Pi Agent 的真实模型调用需配置服务端密钥。图片附件、口播音频与图片生成、对话驱动的时间线调整、默认模型设置、Logo 菜单及其子页面仍是待开发需求，不应在验收时视为已有功能。
...
采用单列内容流与底部常驻输入框，优先呈现当前对话和最新产物。素材、图片、音频、视频以可点击卡片展示，长内容按需展开；次级页面从 Logo 菜单进入，返回后保留会话位置与未发送草稿。按钮和卡片提供适合手指操作的点击区域，输入框在软键盘弹出时仍可见，并适配安全区与不同屏宽。上传和生成任务在离开当前页面后仍可查看进度；弱网或失败时保留输入和已完成结果，提供重试。对话背景图片不得降低文字、状态与控件的可读性。
...
6. 点击视频预览或成片卡片，进入「剪辑拼接」详情页查看片段与口播音轨；如需裁剪、分割、排序或配音，仍通过对话发送要求。
...
- Harness 使用 Pi Agent。Agent Loop 在服务端读取会话上下文、选择并调用受校验的工具、检查结果，再决定回复或继续执行；其能力覆盖剪辑方案、口播文案与音频生成、图片生成和成片任务编排。FFmpeg 执行确定性的剪辑与合成，服务端保管模型密钥，H5 不直接调用模型服务。若尚未接入画面理解能力，不能声称已自动识别真实“精彩镜头”。
